...
const CiceroFolder = ({ onBack, onStartStudy }) => {
  ...
  const ciceroCards = useMemo(() => 
    cards.filter(card => card.curriculum === 'CICERO'), 
    [cards]
  );
  ...
  const filteredCards = useMemo(() => {
    ...
    if (filterMode === 'chapter' || filterMode === 'alphabetical') {
      ...
      return filterCardsByPartOfSpeech(ciceroCards, filterMode);
    }
  }, [ciceroCards, filterMode]);
  ...
  const partsOfSpeech = useMemo(() => 
    getPartsOfSpeechWithCounts(ciceroCards),
    [ciceroCards]
  );
  ...
  // Get all chapters for Cicero curriculum
  const allLessons = useMemo(() => {
    const lessons = ciceroCards.map(card => card.lesson_number);
              ...
              {chapterCards.map((card, index) => (
                <div key={card.id} className="vocabulary-item">
                  <div className="vocabulary-word">
                    <span className="chapter-badge">CIC {card.lesson_number}</span>
                  ...
                  <div className="vocabulary-status">
                    <span className="status-new">CIC</span>
                  </div>
                </div>
              ))}
            </div>
  ...
  return (
    <div className="ludus-folder">
      ...
      <h1 className="main-title">CICERO</h1>
